...
def index_segment(point_1, point_2):
    ...
    z_int = int_between(z_start, z_end) # List of integers the y-value passes through
    ...
    if x_start != x_end:
        for x_cord in x_int:
            ...
            target_point = Point3(target[0], target[1], target[2])
...
def avoid_grid(points):
    ...
    for pt in points:
        ...
        is_z_int = abs(int(pt.z) - pt.z) < 0.00001
...
def run(points, dimension, close):
    ...
    while(has_grid_point(refined_points)):
        refined_points = avoid_grid(refined_points)
